<script setup>
const contactInfo = ref({
  email: "support@egycafe.com",
  phone: "01035478124",
});

const socials = ref([
  {
    title: "Instagram",
    icon: `
        <svg width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g clip-path="url(#clip0_1115_17233)">
            <path d="M7.3229 0.087805C5.99269 0.150305 5.08436 0.362805 4.29165 0.674263C3.45802 0.990046 2.70307 1.4834 2.07915 2.1201C1.44508 2.74574 0.954593 3.50173 0.641653 4.33572C0.334361 5.13156 0.124986 6.04197 0.066653 7.37114C0.00831964 8.70031 -0.00522203 9.12947 0.00102797 12.5243C0.00727797 15.9191 0.022903 16.3441 0.0874863 17.6774C0.151028 19.0076 0.362486 19.9159 0.673945 20.7097C0.994778 21.5316 1.42394 22.2274 2.11978 22.9211C2.74602 23.5555 3.50275 24.046 4.33749 24.3586C5.13228 24.6659 6.04165 24.8753 7.37186 24.9336C8.70311 24.992 9.13124 25.0055 12.525 24.9993C15.9187 24.993 16.3458 24.9774 17.6781 24.9138C19.0114 24.8513 19.9146 24.6378 20.7094 24.3274C21.5426 24.0115 22.2972 23.5181 22.9208 22.8816C23.5551 22.2553 24.0456 21.4986 24.3583 20.6638C24.6656 19.8691 24.875 18.9597 24.9323 17.6305C24.9906 16.2972 25.0052 15.8701 24.9979 12.4763C24.9916 9.0826 24.9771 8.65656 24.9135 7.32322C24.851 5.98989 24.6385 5.08572 24.326 4.29093C24.0104 3.45777 23.5174 2.70319 22.8812 2.07947C22.2553 1.44528 21.4989 0.954778 20.6646 0.641972C19.8687 0.335722 18.9604 0.125305 17.6292 0.069055C16.2989 0.00967995 15.8708 -0.00594506 12.476 0.000304942C9.08124 0.00655494 8.65624 0.0221799 7.3229 0.087805ZM7.46874 22.6847C6.24999 22.6326 5.58853 22.4295 5.1479 22.2597C4.60411 22.061 4.11207 21.7423 3.70832 21.3274C3.29201 20.9254 2.97159 20.4348 2.77082 19.892C2.59894 19.4513 2.39269 18.7899 2.33644 17.5711C2.27394 16.2545 2.26144 15.8586 2.25311 12.5211C2.24686 9.18364 2.25936 8.78885 2.31665 7.47114C2.36874 6.25343 2.5729 5.59093 2.74165 5.15031C2.96665 4.56593 3.23853 4.15031 3.67394 3.71072C4.07626 3.29429 4.56724 2.97385 5.1104 2.77322C5.55103 2.60135 6.21144 2.39718 7.43019 2.33885C8.7479 2.27635 9.14269 2.26385 12.4802 2.25551C15.8167 2.24926 16.2125 2.26176 17.5323 2.3201C18.7489 2.37218 19.4114 2.57426 19.8521 2.7451C20.4354 2.9701 20.8521 3.23989 21.2917 3.67739C21.7312 4.11489 22.001 4.52843 22.2292 5.11281C22.401 5.55239 22.6062 6.21281 22.6635 7.4326C22.726 8.75031 22.7406 9.14614 22.7469 12.4826C22.7521 15.8191 22.7406 16.2149 22.6833 17.5326C22.6302 18.7513 22.4281 19.4128 22.2583 19.8555C22.0333 20.4388 21.7614 20.8555 21.325 21.293C20.923 21.7093 20.4324 22.0297 19.8896 22.2305C19.45 22.4024 18.7875 22.6076 17.5708 22.6659C16.2521 22.7284 15.8573 22.7409 12.5187 22.7482C9.18019 22.7555 8.78749 22.7409 7.46874 22.6847ZM17.6594 5.81906C17.6598 6.11577 17.7482 6.4057 17.9134 6.65217C18.0786 6.89863 18.3132 7.09056 18.5875 7.20368C18.8618 7.3168 19.1635 7.34603 19.4544 7.28766C19.7453 7.2293 20.0124 7.08596 20.2219 6.87579C20.4313 6.66561 20.5737 6.39804 20.6311 6.10692C20.6884 5.8158 20.6581 5.51421 20.5441 5.2403C20.43 4.96639 20.2373 4.73246 19.9902 4.5681C19.7432 4.40375 19.4529 4.31635 19.1562 4.31697C18.7586 4.3178 18.3775 4.47649 18.0969 4.75815C17.8162 5.03982 17.6588 5.42141 17.6594 5.81906ZM6.08228 12.5128C6.08559 14.215 6.76498 15.8462 7.97097 17.0475C9.17697 18.2488 10.8108 18.9219 12.513 18.9185C14.2152 18.9152 15.8464 18.2358 17.0477 17.0298C18.249 15.8238 18.9221 14.19 18.9187 12.4878C18.8942 10.7996 18.2055 9.18909 17.0018 8.00511C15.7982 6.82114 14.1765 6.15909 12.4882 6.16237C10.7998 6.16566 9.18073 6.83403 7.9817 8.02268C6.78267 9.21133 6.10024 10.8245 6.08228 12.5128ZM8.33332 12.5086C8.33167 11.6846 8.57443 10.8785 9.0309 10.1924C9.48737 9.50625 10.137 8.9709 10.8978 8.65401C11.6585 8.33712 12.4961 8.25294 13.3047 8.41209C14.1133 8.57125 14.8565 8.9666 15.4404 9.54815C16.0242 10.1297 16.4226 10.8713 16.5849 11.6793C16.7473 12.4872 16.6665 13.3251 16.3527 14.0871C16.0388 14.8491 15.5061 15.5009 14.8218 15.9601C14.1375 16.4193 13.3324 16.6653 12.5083 16.667C11.9611 16.6681 11.4191 16.5614 10.9132 16.353C10.4072 16.1446 9.94728 15.8386 9.5596 15.4525C9.17191 15.0663 8.86408 14.6076 8.65367 14.1025C8.44327 13.5974 8.33441 13.0558 8.33332 12.5086Z" fill="#C1D4EE"/>
            </g>
            <defs>
            <clipPath id="clip0_1115_17233">
            <rect width="25" height="25" fill="white"/>
            </clipPath>
            </defs>
        </svg>
    `,
    link: "https://www.instagram.com",
  },
  {
    title: "X - Twitter",
    icon: `
        <svg width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M14.5219 10.825L22.4328 1.5625H20.5578L13.6906 9.60469L8.20313 1.5625H1.875L10.1719 13.725L1.875 23.4375H3.75L11.0031 14.9438L16.7984 23.4375H23.1266L14.5219 10.825ZM11.9547 13.8313L11.1141 12.6203L4.425 2.98438H7.30469L12.7016 10.7609L13.5422 11.9719L20.5594 22.0812H17.6797L11.9547 13.8313Z" fill="#C1D4EE"/>
        </svg>
    `,
    link: "https://www.x.com",
  },
  {
    title: "Facebook",
    icon: `
    <svg width="24" height="25" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg">
        <g clip-path="url(#clip0_1115_17229)">
        <path d="M24 12.5C24 5.87258 18.6274 0.5 12 0.5C5.37258 0.5 0 5.87258 0 12.5C0 18.4895 4.3882 23.454 10.125 24.3542V15.9688H7.07812V12.5H10.125V9.85625C10.125 6.84875 11.9166 5.1875 14.6576 5.1875C15.9701 5.1875 17.3438 5.42188 17.3438 5.42188V8.375H15.8306C14.34 8.375 13.875 9.30008 13.875 10.25V12.5H17.2031L16.6711 15.9688H13.875V24.3542C19.6118 23.454 24 18.4895 24 12.5Z" fill="#C1D4EE"/>
        </g>
        <defs>
        <clipPath id="clip0_1115_17229">
        <rect width="24" height="24" fill="white" transform="translate(0 0.5)"/>
        </clipPath>
        </defs>
    </svg>
    `,
    link: "https://www.facebook.com",
  },
  {
    title: "Whatsapp",
    icon: `
    <svg width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M19.8437 5.11431C18.8886 4.14984 17.751 3.38511 16.4974 2.86469C15.2437 2.34427 13.899 2.07857 12.5416 2.08306C6.85413 2.08306 2.21871 6.71848 2.21871 12.406C2.21871 14.2289 2.69788 15.9997 3.59371 17.5622L2.13538 22.9164L7.60413 21.4789C9.11454 22.3018 10.8125 22.7393 12.5416 22.7393C18.2291 22.7393 22.8645 18.1039 22.8645 12.4164C22.8645 9.65598 21.7916 7.06223 19.8437 5.11431ZM12.5416 20.9893C11 20.9893 9.48954 20.5726 8.16663 19.7914L7.85413 19.6039L4.60413 20.4581L5.46871 17.2914L5.26038 16.9685C4.40365 15.6008 3.94884 14.0198 3.94788 12.406C3.94788 7.67681 7.80204 3.82265 12.5312 3.82265C14.8229 3.82265 16.9791 4.71848 18.5937 6.34348C19.3933 7.13917 20.027 8.08571 20.4579 9.1282C20.8888 10.1707 21.1085 11.2884 21.1041 12.4164C21.125 17.1456 17.2708 20.9893 12.5416 20.9893ZM17.25 14.5726C16.9895 14.4476 15.7187 13.8226 15.4895 13.7289C15.25 13.6456 15.0833 13.6039 14.9062 13.8539C14.7291 14.1143 14.2395 14.6976 14.0937 14.8643C13.9479 15.0414 13.7916 15.0622 13.5312 14.9268C13.2708 14.8018 12.4375 14.5206 11.4583 13.6456C10.6875 12.9581 10.177 12.1143 10.0208 11.8539C9.87496 11.5935 9.99996 11.4581 10.1354 11.3226C10.25 11.2081 10.3958 11.0206 10.5208 10.8747C10.6458 10.7289 10.6979 10.6143 10.7812 10.4476C10.8645 10.2706 10.8229 10.1247 10.7604 9.99973C10.6979 9.87473 10.177 8.6039 9.96871 8.08306C9.76038 7.58306 9.54163 7.64556 9.38538 7.63515H8.88538C8.70829 7.63515 8.43746 7.69765 8.19788 7.95806C7.96871 8.21848 7.30204 8.84348 7.30204 10.1143C7.30204 11.3851 8.22913 12.6143 8.35413 12.781C8.47913 12.9581 10.177 15.5622 12.7604 16.6768C13.375 16.9476 13.8541 17.1039 14.2291 17.2185C14.8437 17.4164 15.4062 17.3851 15.8541 17.3226C16.3541 17.2497 17.3854 16.6976 17.5937 16.0935C17.8125 15.4893 17.8125 14.9789 17.7395 14.8643C17.6666 14.7497 17.5104 14.6976 17.25 14.5726Z" fill="#C1D4EE"/>
    </svg>
    `,
    link: "https://wa.me/201012345678",
  },
]);

function goToElement(Id) {
  document.getElementById(Id).scrollIntoView({
    behavior: "smooth",
  });
}
</script>

<template>
  <footer class="footer">
    <div class="footer-container">
      <div class="content">
        <div class="part upper">
          <div class="sub-part general-info">
            <img src="/images/logo.svg" alt="" />

            <p class="text">
              نظام متكامل لإدارة شبكات الواي فاي في الكافيهات والمؤسسات
              التجارية.
            </p>

            <div class="socials">
              <a
                class="social"
                v-for="social in socials"
                :key="social.title"
                :href="social.link"
                v-html="social.icon"
                target="_blank"
              ></a>
            </div>
          </div>

          <div class="sub-part fast-links">
            <h6 class="title">روابط سريعة</h6>

            <ul class="links">
              <li class="link" @click="goToElement('features')">المميزات</li>
              <li class="link" @click="goToElement('pricing')">الاسعار</li>
              <li class="link" @click="goToElement('faq')">الاسلئله الشائعه</li>
              <li class="link" @click="goToElement('contact')">تواصل معنا</li>
            </ul>
          </div>

          <div class="sub-part legal">
            <h6 class="title">قانونى</h6>

            <ul class="links">
              <li class="link"><a href="#">سياسة الخصوصية</a></li>
              <li class="link"><a href="#">شروط الاستخدام</a></li>
            </ul>
          </div>

          <div class="sub-part contact-us">
            <h6 class="title">تواصل معنا</h6>

            <ul class="links">
              <li class="link">
                <a :href="`mailto:${contactInfo.email}`" target="_blank">
                  <img src="/icons/email.svg" alt="-" />
                  <p>
                    {{ contactInfo.email }}
                  </p>
                </a>
              </li>
              <li class="link">
                <a :href="`tel:${contactInfo.phone}`" target="_blank">
                  <img src="/icons/phone.svg" alt="-" />
                  <p>
                    {{ contactInfo.phone }}
                  </p>
                </a>
              </li>
            </ul>

            <div class="bank-accounts">
              <a class="account" href="#">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                >
                  <g clip-path="url(#clip0_1115_17197)">
                    <path
                      d="M9.112 8.262L5.97 15.758H3.92L2.374 9.775C2.28 9.407 2.199 9.272 1.913 9.117C1.447 8.864 0.677 8.627 0 8.479L0.046 8.262H3.346C3.79156 8.26155 4.171 8.58582 4.24 9.026L5.057 13.364L7.075 8.262M17.145 13.311C17.153 11.332 14.409 11.223 14.428 10.339C14.434 10.07 14.69 9.784 15.25 9.711C15.9063 9.64867 16.5672 9.76476 17.163 10.047L17.503 8.457C16.923 8.23895 16.3087 8.12618 15.689 8.124C13.772 8.124 12.423 9.144 12.411 10.603C12.399 11.682 13.374 12.283 14.109 12.643C14.865 13.01 15.119 13.246 15.115 13.574C15.11 14.078 14.513 14.299 13.955 14.308C12.98 14.323 12.415 14.045 11.963 13.835L11.612 15.477C12.065 15.685 12.901 15.867 13.768 15.875C15.805 15.875 17.138 14.869 17.145 13.311ZM22.206 15.758H24L22.435 8.262H20.779C20.4169 8.25865 20.0895 8.47666 19.953 8.812L17.044 15.758H19.08L19.485 14.638H21.973M20.043 13.102L21.063 10.287L21.651 13.102M11.883 8.262L10.28 15.758H8.34L9.945 8.262"
                      fill="white"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_1115_17197">
                      <rect width="24" height="24" fill="white" />
                    </clipPath>
                  </defs>
                </svg>
              </a>

              <a class="account" href="#">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                >
                  <g clip-path="url(#clip0_1115_17193)">
                    <path
                      d="M11.343 18.031C11.401 18.08 11.463 18.129 11.524 18.177C10.3075 18.9866 8.87828 19.4174 7.417 19.415C4.24234 19.4149 1.41983 17.3942 0.396764 14.3889C-0.626298 11.3836 0.377033 8.06041 2.89228 6.12339C5.40752 4.18638 8.87673 4.06521 11.521 5.822C11.461 5.873 11.401 5.92 11.356 5.972C9.60314 7.48443 8.59528 9.68484 8.595 12C8.59552 14.3132 9.59776 16.5128 11.343 18.031ZM16.584 4.584C15.064 4.584 13.653 5.04 12.479 5.822C12.539 5.873 12.599 5.92 12.644 5.972C14.3969 7.48443 15.4047 9.68484 15.405 12C15.4037 14.313 14.4016 16.5123 12.657 18.031C12.599 18.08 12.537 18.129 12.476 18.177C13.6924 18.9869 15.1217 19.4177 16.583 19.415C18.55 19.4158 20.4367 18.6349 21.8278 17.2442C23.2189 15.8535 24.0003 13.967 24 12C23.9994 7.90449 20.6795 4.58455 16.584 4.584ZM12 6.174C10.2099 7.57676 9.16567 9.72578 9.169 12C9.16506 14.2746 10.2094 16.4242 12 17.827C13.7901 16.4238 14.8343 14.2745 14.831 12C14.8343 9.72578 13.7901 7.57676 12 6.174Z"
                      fill="white"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_1115_17193">
                      <rect width="24" height="24" fill="white" />
                    </clipPath>
                  </defs>
                </svg>
              </a>

              <a class="account" href="#">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                >
                  <g clip-path="url(#clip0_1115_17189)">
                    <path
                      d="M2.15 4.318C1.99866 4.31818 1.84733 4.31919 1.696 4.321C1.546 4.326 1.393 4.334 1.244 4.361C0.787199 4.43319 0.392866 4.72038 0.184 5.133C0.114 5.271 0.07 5.411 0.044 5.563C0.016 5.711 0.007 5.863 0.004 6.013C0.00195281 6.08265 0.000619407 6.15232 0 6.222V17.779C0 17.849 0.002 17.917 0.003 17.986C0.007 18.136 0.016 18.289 0.043 18.438C0.07 18.588 0.115 18.729 0.185 18.867C0.322895 19.1385 0.543517 19.3591 0.815 19.497C0.953 19.567 1.093 19.612 1.245 19.639C1.393 19.666 1.545 19.675 1.695 19.679L1.903 19.682H22.097L22.304 19.679C22.454 19.675 22.607 19.666 22.756 19.639C22.906 19.612 23.047 19.568 23.184 19.498C23.4562 19.3602 23.6772 19.1392 23.815 18.867C23.885 18.729 23.93 18.589 23.956 18.437C23.983 18.289 23.992 18.137 23.996 17.987C23.998 17.917 23.999 17.849 23.999 17.779L24 17.533V6.221C24 6.151 23.998 6.083 23.996 6.014C23.9941 5.86254 23.9807 5.71144 23.956 5.562C23.8579 4.94426 23.3737 4.45966 22.756 4.361C22.6065 4.33636 22.4555 4.32299 22.304 4.321C22.153 4.31673 22.002 4.31573 21.851 4.318M2.15 4.83H22.092C22.158 4.83 22.223 4.832 22.289 4.833C22.404 4.837 22.539 4.843 22.664 4.865C22.773 4.885 22.864 4.915 22.951 4.959C23.1264 5.04804 23.269 5.19058 23.358 5.366C23.4038 5.45665 23.4355 5.55377 23.452 5.654C23.474 5.777 23.48 5.912 23.483 6.028C23.485 6.093 23.486 6.158 23.486 6.225V17.777C23.486 17.842 23.486 17.907 23.483 17.973C23.48 18.088 23.474 18.223 23.451 18.348C23.4071 18.6466 23.2206 18.9052 22.951 19.041C22.8609 19.0866 22.7645 19.1182 22.665 19.135C22.5416 19.1546 22.4169 19.1653 22.292 19.167L22.092 19.17H1.906C1.84 19.17 1.773 19.168 1.71 19.167C1.58439 19.1654 1.45907 19.1547 1.335 19.135C1.226 19.115 1.135 19.085 1.047 19.041C0.871582 18.9523 0.729268 18.8096 0.641 18.634C0.595308 18.5433 0.563616 18.4462 0.547 18.346C0.52714 18.2226 0.516444 18.098 0.515 17.973C0.513659 17.9073 0.512992 17.8417 0.513 17.776V6.224C0.513 6.159 0.513 6.093 0.515 6.027C0.519 5.913 0.525 5.779 0.547 5.652C0.567 5.544 0.597 5.453 0.641 5.365C0.730102 5.18987 0.872655 5.04767 1.048 4.959C1.13853 4.91374 1.23523 4.88206 1.335 4.865C1.46 4.843 1.595 4.836 1.71 4.833C1.775 4.831 1.841 4.831 1.906 4.83M6.86 8.53C6.56 8.546 6.192 8.729 5.98 8.986C5.789 9.206 5.62 9.566 5.664 9.904C6.002 9.934 6.339 9.735 6.552 9.486C6.757 9.228 6.897 8.883 6.86 8.531M9.067 8.95V14.443H9.919V12.566H11.099C12.177 12.566 12.934 11.827 12.934 10.754C12.934 9.684 12.192 8.949 11.126 8.949M9.919 9.669H10.901C11.64 9.669 12.062 10.065 12.062 10.758C12.062 11.45 11.64 11.85 10.898 11.85H9.919M6.765 9.969C6.315 9.979 5.935 10.249 5.715 10.249C5.48 10.249 5.122 9.985 4.734 9.992C4.22041 10.0048 3.75219 10.2892 3.504 10.739C2.977 11.647 3.365 12.994 3.878 13.734C4.127 14.1 4.427 14.503 4.822 14.488C5.195 14.474 5.342 14.246 5.795 14.246C6.249 14.246 6.381 14.488 6.775 14.481C7.185 14.474 7.442 14.115 7.69 13.748C7.976 13.331 8.093 12.928 8.1 12.907C8.093 12.899 7.31 12.599 7.303 11.698C7.295 10.944 7.918 10.585 7.947 10.563C7.595 10.043 7.047 9.985 6.857 9.97C6.82637 9.96808 6.79568 9.96741 6.765 9.968M14.969 10.366C13.979 10.366 13.363 10.899 13.317 11.622H14.094C14.166 11.264 14.463 11.036 14.939 11.036C15.441 11.036 15.742 11.302 15.742 11.747V12.056L14.645 12.12C13.694 12.174 13.157 12.604 13.157 13.304C13.157 14.024 13.705 14.511 14.489 14.511C15.015 14.511 15.521 14.23 15.753 13.784H15.772V14.443H16.56V11.683C16.56 10.88 15.94 10.366 14.969 10.366ZM16.909 10.438L18.355 14.447C18.355 14.45 18.282 14.687 18.282 14.694C18.157 15.104 17.952 15.265 17.571 15.265C17.502 15.265 17.365 15.265 17.304 15.25V15.916C17.364 15.927 17.571 15.935 17.639 15.935C18.469 15.935 18.865 15.623 19.207 14.652L20.707 10.438H19.839L18.827 13.697H18.812L17.799 10.437M15.742 12.627V12.943C15.742 13.464 15.292 13.86 14.718 13.86C14.276 13.86 13.987 13.632 13.987 13.281C13.987 12.939 14.265 12.721 14.756 12.688"
                      fill="white"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_1115_17189">
                      <rect width="24" height="24" fill="white" />
                    </clipPath>
                  </defs>
                </svg>
              </a>
            </div>
          </div>
        </div>
        <div class="part lower">
          <p class="rights">
            نظام إدارة كروت الواي فاي. جميع الحقوق&copy; محفوظة.
          </p>
        </div>
      </div>
    </div>
  </footer>
</template>

<style scoped>
.footer {
  background-color: #10162e;
  border-top: 1px solid #1d293d;

  & .footer-container {
    width: 100%;
    height: 100%;
    background-color: #171b25;
    padding-top: 65px;

    & .content {
      display: flex;
      flex-direction: column;
      justify-content: space-between;
      align-items: center;
      width: 100%;
      height: 100%;
      min-height: max-content;
      padding: 0 25px;
      gap: 50px;

      & .part {
        background-color: transparent;

        & .title {
          color: #fff;
          text-align: right;
          font-size: 16px;
          font-weight: 600;
          line-height: 24px;
        }

        &.upper {
          display: flex;
          flex-direction: row;
          justify-content: space-between;
          align-items: flex-start;
          width: 85%;
          gap: 30px;

          & .sub-part {
            display: flex;
            flex-direction: column;
            justify-content: center;
            align-items: flex-start;
            gap: 20px;

            p {
              color: #90a1b9;
              text-align: right;
              font-size: 14px;
              font-weight: 400;
              line-height: 20px;
            }

            & .links {
              display: flex;
              flex-direction: column;
              justify-content: space-between;
              align-items: flex-start;
              gap: 2px;

              & .link {
                color: #90a1b9;
                font-size: 16px;
                font-weight: 400;
                line-height: 24px;
                cursor: pointer;

                &:hover {
                  filter: brightness(1.3);
                }

                & a {
                  display: flex;
                  flex-direction: row;
                  justify-content: space-between;
                  align-items: center;
                  flex-wrap: nowrap;
                  gap: 6px;
                }
              }
            }

            &.general-info {
              & .text {
                width: 240px;
              }

              & .socials {
                display: flex;
                justify-content: space-between;
                align-items: center;
                width: 80%;
                gap: 10px;

                & .social {
                  width: 25px;
                  height: 25px;

                  &:hover {
                    scale: 1.02;
                    filter: brightness(2);
                  }

                  &:active {
                    scale: 0.9;
                  }
                }
              }
            }

            &.fast-links {
              justify-content: space-between;
            }

            &.contact-us {
              & .bank-accounts {
                display: flex;
                flex-direction: row;
                justify-content: space-between;
                align-items: center;
                gap: 12px;

                & .account {
                  &:hover {
                    scale: 1.02;
                    filter: brightness(1.3);
                  }
                }
              }
            }
          }
        }

        &.lower {
          width: 98%;
          padding: 30px 0;
          border-top: 1px solid #1d293d;

          & .rights {
            color: #b1bac6;
            text-align: center;
            font-size: 16px;
            font-weight: 400;
            line-height: 20px;
          }
        }
      }
    }
  }
}
</style>
